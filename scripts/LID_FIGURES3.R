...
ggsave(sprintf("%sFIGURE2.jpg",out_path), es.heatmap,
       height = 70, width = one.c, units = 'mm',
       dpi = 300)


##### FIGURE 3: SOURCE NORMALIZATION
...
dat.sn$name[!is.na(dat.sn$fitness) & dat.sn$method == 2] <- '2. LID-SN'
...
sn.sn <- ggplot(dat.sn[!is.na(dat.sn$fitness) & dat.sn$method == 3,],
                aes(x = source, y = fitness)) +
  geom_boxplot(width = 0.5) +
  geom_violin(fill = 'transparent') +
  stat_compare_means(label.x = 4.5, label.y = 1, hjust = 0.5, size = 1.5) +
...
sn.bean <- ggplot(dat.sn[!is.na(dat.sn$fitness) & dat.sn$method == 4,],
                  aes(x = source, y = fitness)) +
  geom_boxplot(width = 0.5) +
  geom_violin(fill = 'transparent') +
  stat_compare_means(label.x = 4.5, label.y = 1, hjust = 0.5, size = 1.5) +
...
fig3 <- ggarrange(sn.raw,sn.nosn,sn.sn,sn.bean,
                  nrow = 1, ncol = 4,
                  labels = c('a','','',''),
...
##### FIGURE 4: RMSE and BACKGROUND PREDICTION
...
### FINAL FIGURE
fig4 <- ggpubr::ggarrange(pred.lid, pred.nocc, pred.nosn,
...
fig3.5 <- ggpubr::ggarrange(fig3, fig4, ncol = 1, nrow = 2,
                            heights = c(1,2))
ggsave(sprintf("%sFIGURE3_5.jpg",out_path), fig3.5,
       height = 180, width = two.c, units = 'mm',
       dpi = 300)


##### FIGURE 5: OVERALL PERFORMACE
...
spe.1 <- ggplot(stats.tmp[stats.tmp$hours == stats.tmp$cont_hrs,],
  ...
  stat_summary(fun.data=mean_sdl, fun.args = list(mult=1),
               geom="errorbar", color="black", lwd = 0.5) +
  stat_summary(fun.y=mean, geom="point", color="#FFC107", size = 2) +
  labs(x = "p",
       y = "Specificity") +
  scale_x_continuous(breaks = seq(0,1,0.025),
                     minor_breaks = seq(0,1,0.0125)) +
  scale_y_continuous(breaks = seq(0,100,1),
                     minor_breaks = seq(0,100,0.5),
                     labels = paste(sprintf('%d',seq(0,100,1)),'%', sep = '')) +
...
fig5 <- ggarrange(spe.1, sen.fdr, sen.met, sen.rep,
                  nrow = 2, ncol = 2,
                  labels = c('a','b','c','d'),
                  label.args = list(gp=gpar(font = 2, fontsize = lbls),
                                    hjust=-1))
...
fig6 <- ggarrange(rnd_den, rnd_pie,
