...
data.fit.all$average[data.fit.all$row %in% c(1,2,3,4,61,62,63,64) | 
                       data.fit.all$col %in% c(1,2,3,4,93,94,95,96)] <- NA
...
data.fit.all$fitness[data.fit.all$row %in% c(1,2,3,4,61,62,63,64) | 
                       data.fit.all$col %in% c(1,2,3,4,93,94,95,96)] <- NA
...
data.fit.all$source[data.fit.all$row%%2==1 & data.fit.all$col%%2==1] = 'TL'
data.fit.all$source[data.fit.all$row%%2==0 & data.fit.all$col%%2==1] = 'BL'
data.fit.all$source[data.fit.all$row%%2==1 & data.fit.all$col%%2==0] = 'TR'
data.fit.all$source[data.fit.all$row%%2==0 & data.fit.all$col%%2==0] = 'BR'
...
data.temp <- data.fit.all %>% #[,c('arm', 'condition', 'pos', 'rep', 'strain_id', 'orf_name', 'category', 'hours', 'norm_cs')]
  group_by(arm, condition, rep, strain_id, orf_name, category, hours) %>%
  summarize(norm_cs = median(norm_cs, na.rm = T), .groups = 'keep') %>%
  data.frame()
...
#       s <- unique(data.temp$strain_id[data.temp$arm == a & data.temp$condition == c & data.temp$rep == r])
...
#     save(data.pred, file = sprintf('output/translatome/data.pred.%s.%s.RData',a,c))
#     
#     ## GROWTH CURVE ANALYSIS
#     temp.gr.res <- NULL
...
# ##### REFERENCE LIMITS
# head(data.gcgr.res)
# data.gcgr.lim <- data.gcgr.res[,c(-3,-4,-6,-7,-15,-16)] %>%
#   filter(orf_name == 'BF_control') %>%
...
#   data.frame()
# 
# data.gcgr.sum <- melt(data.gcgr.res[!is.na(data.gcgr.res$category),c(-7,-15,-16)] %>%
#        filter(category != 'Reference'), 
#        id.vars = c('arm','condition','rep','strain_id','orf_name','category'))
# data.gcgr.sum <- merge(data.gcgr.sum, data.gcgr.lim[,-3], by = c('arm','condition','variable'), all.x = T)
# 
...
data.fit.sum <- data.fit.all %>%
  ...
  summarize(fitness = median(fitness, na.rm = T),
            ...
            norm_cs = median(norm_cs, na.rm = T),
            .groups = 'keep') %>%
  data.frame()
...
write.csv(data.fit.all[,c(1:3,5:9,11,12,29)], file = 'output/translatome/tr_oe_fitandcs_highres_all.csv')

temp <- data.fit.all[order(data.fit.all$arm, data.fit.all$condition, data.fit.all$hours,
                           data.fit.all$plate, data.fit.all$col, data.fit.all$row),
                     c(1:16)]
dbWriteTable(conn, 'TR_OE_FS_ALL_6144_CLEAN_FITNESS', temp, overwrite = T)

# 220106.RData has data up until this point without the .gcgr data

##### PLOTTING GROWTH DYNAMICS
## GROWTH CURVES
data.pred.files <- list.files('/home/sbp29/R/Projects/adaptivefitness/output/translatome', 'data.pred', recursive=F, full.names=T, include.dirs=TRUE)
data.pred.files <- data.pred.files[!str_detect(data.pred.files, '.pos')]

data.fit.pred <- NULL
for (f in data.pred.files) {
  load(f)
  temp <- NULL
  for (i in 1:dim(data.pred)[2]) {
    temp <- rbind(temp, data.pred[,i])
  }
  temp <- data.frame(colnames(data.pred), temp)
  data.fit.pred <- rbind(data.fit.pred, temp)
}
head(data.fit.pred)
temp <- str_split(data.fit.pred$colnames.data.pred., ',', simplify = T) %>% data.frame(stringsAsFactors = F)
colnames(temp) <- c('arm','condition','rep','strain_id')
temp$rep <- as.numeric(temp$rep)
temp$strain_id <- as.numeric(temp$strain_id)
data.fit.pred <- cbind(temp, data.fit.pred)

temp <- data.fit.pred[,-5] %>% 
  filter(arm == 'Time') %>%
  melt(id.vars = c('arm', 'condition', 'rep', 'strain_id'),
       value.name = 'Time')
temp$condition <- c('GA','HO','HU','SA','DM','FL','TN')

data.fit.pred <- merge(temp[,c(2,5,6)], data.fit.pred[,-5] %>% 
                         filter(arm != 'Time') %>%
                         melt(id.vars = c('arm', 'condition', 'rep', 'strain_id'),
                              value.name = 'norm_cs'),
                       by = c('condition','variable'))

data.fit.pred <- merge(data.strains[,-7], data.fit.pred[,-2], 
      by = c('arm','condition','rep','strain_id'))

data.fit.pred %>%
  ggplot(aes(x = Time, y = norm_cs)) +
  geom_line(aes(group = rep, col = category)) +
...
## TIME TO ASSESS OE FITNESS
data.fit.besttime <- merge(data.fit.pred %>%
  group_by(condition, Time) %>%
  summarize(max_cs = max(norm_cs, na.rm = T)) %>%
  filter(max_cs <= 1800) %>%
  group_by(condition) %>%
  summarize(max_time = max(Time, na.rm = T)) %>%
  data.frame(), data.fit.all %>%
  group_by(condition, hours) %>%
  count() %>% data.frame(), by = 'condition') %>%
  filter(hours <= round(max_time)) %>%
  group_by(condition, max_time) %>%
  summarise(fit_hour = max(hours)) %>%
  data.frame()

data.fit <- merge(data.fit.all,data.fit.besttime[,c(1,3)], by = 'condition') %>% filter(hours == fit_hours) %>% data.frame()

## GROWTH VARIABLES
data.gcgr.res[!is.na(data.gcgr.res$category),] %>%
  ggplot(aes(x = auc_l, y = category)) +
  geom_density_ridges() +
  facet_wrap(.~condition, scales = 'free_x')
...
# 211220.RData has data up until this point
...
##### BENEFICIAL HITS BASED ON GR ACCORDING TO AARON
oe.gr.hits <- read_delim(file = 'output/translatome/oe_mutant_highly_beneficial_gr.txt', delim = ' ')

# temp <- merge(data.fit.all, data.fit.lim[,-4], by = c('arm','condition','hours'))
fig.gcgr.hits <- data.fit.sum %>%
  filter(category == 'Transient') %>%
  ggplot(aes(x = hours, y = norm_cs)) +
  # geom_line(aes(group = strain_id, col = 'All Transient')) +
  # geom_line(data = merge(data.fit.sum, oe.gr.hits[,c(2:6)], by = c('arm','condition','strain_id','orf_name')),
  #                        aes(x = hours, y = norm_cs, col = 'Hits', group = strain_id)) +
  # stat_summary(fun.data=mean_sdl, fun.args = list(mult=1),
  #               aes(fill = 'All Transient'), geom="ribbon", alpha = 0.4) + #aes(group = category, fill = category),
  stat_summary(fun=mean, aes(col = 'All Transient'), geom="line", lwd =0.7) + #aes(group = category, col = category),
  # stat_summary(data = merge(data.fit.sum, oe.gr.hits[,c(2:6)], by = c('arm','condition','strain_id','orf_name')),
  #              fun.data=mean_sdl, fun.args = list(mult=1),
  #              aes(x = hours, y = norm_cs, group = category, fill = 'Hits'), geom="ribbon", alpha = 0.4) +
  stat_summary(data = merge(data.fit.sum, oe.gr.hits[,c(2:6)], by = c('arm','condition','strain_id','orf_name')),
               aes(x = hours, y = norm_cs, group = category, col = 'Hits'), fun=mean, geom="line", lwd =0.7) +
  scale_y_continuous(trans = 'pseudo_log') +
  facet_wrap(.~arm*condition, scale = 'free_x') #+
  # coord_cartesian(xlim = c(0,50),
  #                 ylim = c(0,750))
ggsave(sprintf("%s/GROWTHCURVES_HITS.jpg",fig_path), fig.gcgr.hits,
       height = two.c, width = two.c, units = 'mm',
       dpi = 600)

##### BENEFICIAL HITS BASED ON FITNESS ACCORDING TO AARON
oe.hits <- read_delim(file = 'output/translatome/oe_mutant_highly_beneficial.txt', delim = ' ')
data.fit.hit.all <- merge(data.fit.all, oe.hits[,c(2:7)], by = c('condition','arm','rep','strain_id','orf_name','hours'))
