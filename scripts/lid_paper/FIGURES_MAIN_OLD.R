...
ggsave(sprintf("%sFIGURE2C.jpg",out_path), es.heatmap,
       height = 70, width = 70, units = 'mm',
       dpi = 300)
...
# library(plot3D)
# scatter3D(rmse$hour[order(rmse$method, decreasing = T)],
#           rmse$method[order(rmse$method, decreasing = T)],
#           rmse$per[order(rmse$method, decreasing = T)],
#           phi = 0, bty ="g")
...
fig3a_b <- ggarrange(cv.plot, pred.rmse, 
                     nrow = 1, ncol = 2,
                     labels = c('a','b'),
                     label.args = list(gp=gpar(font = 2, fontsize = lbls),
                                       hjust = -1))
fig3c <- ggarrange(sn.raw,sn.sn,sn.nosn,sn.nocc,sn.bean,
                     nrow = 1, ncol = 5,
                     labels = c('c','','','',''),
                     label.args = list(gp=gpar(font = 2, fontsize = lbls),
                                       hjust = -1))

fig3 <- ggpubr::ggarrange(fig3a_b, fig3c,
                          ncol = 1, nrow = 2,
                          heights = c(1.5,2))
...
fig4 <- ggpubr::ggarrange(spe.res,
          ggpubr::ggarrange(lid.sen.fdr, mcat.sen.fdr,
                            ncol = 2,
                            labels = c('b','c'),
                            font.label = list(face = 'bold', size = lbls),
                            hjust=-1),
...
rnd.es$lid_result <- factor(rnd.es$lid_result, levels = c('Del/Ben',
                                                          'Neu/Ben',
                                                          'Ben/Ben',
                                                          'Ben/Del',
                                                          'Neu/Del',
                                                          'Del/Del'))
rnd.es$mcat_result <- factor(rnd.es$mcat_result, levels = c('Del/Ben',
                                                            'Neu/Ben',
                                                            'Ben/Ben',
                                                            'Ben/Del',
                                                            'Neu/Del',
